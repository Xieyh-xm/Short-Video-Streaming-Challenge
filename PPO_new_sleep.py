...
class ActorCritic(nn.Module):
    ...
    def act(self, state):
        ...
        # 如果第二个视频有buffer，才允许sleep
        if (chunk_last[1] != 0 and buffer[1] != 0) or chunk_last[1] == 0:
            mask[15] = 1
        # 如果没有可下载的块，则不再被允许下载
        for i in range(5):
            for j in range(3):
                if chunk_last[i] != 0.0:
                    mask[i * 3 + j] = 1
        if buffer[0] <= 0.8 and chunk_last[0] != 0:  # 强制下载第一个视频
            mask[15] = 0
            for i in range(3, 15):
                mask[i] = 0
    ...
    def evaluate(self, state, action):
        ...
        for k in range(state_numpy.shape[0]):
            if (chunk_last[k, 1] != 0 and buffer[k, 1] != 0) or chunk_last[k, 1] == 0:
                mask[k, 15] = 1
            for i in range(5):
                for j in range(3):
...
class PPO:
    ...
    def select_action(self, state):
        if self.has_continuous_action_space:
            ...
            if action == 15 or chunk_last[int(action_trans[0])] == 0:  # sleep_flag
                action_trans[2] = True
                action_trans[0] = 0
                action_trans[1] = 0
            else:
