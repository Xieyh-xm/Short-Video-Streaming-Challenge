...
class RLEnv:
    ...
    def step(self, action):
        ...
        if play_video_id > self.last_play_video_id and play_video_id < ALL_VIDEO_NUM:
            ...
            if player_length == 5:  # 如果推荐列表为满，正常读取
                ...
                for i in range(jump_number):  # 删除无用视频
                    ...
                    for i in range(jump_number):
                        ...
                        self.future_videosize.append(size_buffer)
            # 更新当前下载视频的未来10个size
            if download_video_id - self.offset >= 0 and not sleep_flag:  # 下载的视频未被划走
            ...
            # 4.1 更新条件概率
            # 更新正在播放视频的条件概率
            chunk_play_remain = self.net_env.players[play_video_id - self.offset].get_remain_video_num()  # 获取剩余块
            ...
            if download_video_id - self.offset > 0 and not sleep_flag:
                ...
                if chunk_remain > 0:
                    ...
                    self.conditional_retent_rate[download_video_id - self.offset].append(
        ...
        if play_video_id == self.last_play_video_id and not switch_flag:
            ...
            # 下载部分
            # print("play_video_id = "+str(play_video_id))
            # print("download_video_id = " + str(download_video_id))
            # print("self.offset = " + str(self.offset))
            # print("len(self.net_env.players) = " + str(len(self.net_env.players)))
            chunk_remain = self.net_env.players[download_video_id - self.offset].get_remain_video_num()  # 获取剩余块
            if not sleep_flag:
                if chunk_remain < 10:  # 不足10块补0
                    for k in range(3):
