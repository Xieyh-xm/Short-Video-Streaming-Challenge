...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, has_continuous_action_space, action_std_init):
        ...
        if has_continuous_action_space:
            ...
            self.lstm_actor = nn.LSTM(input_size=1, hidden_size=10, num_layers=2, batch_first=True)
            self.actor = nn.Sequential(
        ...
        self.lstm_critic = nn.LSTM(input_size=1, hidden_size=10, num_layers=2, batch_first=True)
    ...
    def act(self, state):
        ...
        if self.has_continuous_action_space:
            ...
            ''' choose action '''
            throughput = state[:, 0:10].unsqueeze(2)
            out, (h, c) = self.lstm_actor(throughput)
            h_out = h[1, :, :]  # 取最后一层10个输出
            tmp_input = torch.cat((h_out, state[:, 10:35]), 1)

            action_probs = self.actor(tmp_input)
    ...
    def evaluate(self, state, action):
        ...
        if self.has_continuous_action_space:
            ...
            ''' choose action '''
            throughput = state[:, 0:10].unsqueeze(2)
            out, (h_actor, c) = self.lstm_actor(throughput)
            h_out_actor = h_actor[1, :, :]  # 取最后一层10个输出
            actor_input = torch.cat((h_out_actor, state[:, 10:35]), 1)
        ...
        throughput = state[:, 0:10].unsqueeze(2)
        out, (h_critic, c) = self.lstm_critic(throughput)
        h_out_critic = h_critic[1, :, :]  # 取最后一层10个输出
        critic_input = torch.cat((h_out_critic, state[:, 10:35]), 1)
