...
class Algorithm:
    ...
    def run(self, delay, rebuf, video_size, end_of_video, play_video_id, Players, first_step=False):
        ...
        if first_step:
            ...
            if play_video_id > self.last_play_video_id and play_video_id < ALL_VIDEO_NUM:
                ...
                if chunk_play_remain >= 10:
                    for i in range(10):
                        ...
                        self.conditional_retent_rate[0][i] = float(Players[0].user_retent_rate[-chunk_play_remain + i - 2])/ float(Players[0].user_retent_rate[max(0, playing_chunk-1)])
                ...
                if self.download_video_id - self.offset > 0 and not sleep_flag:
                    if chunk_remain >= 10: # 直接读
                        for j in range(10):
                            self.conditional_retent_rate[self.download_video_id - self.offset][j] = Players[self.download_video_id - self.offset].user_retent_rate[(-chunk_remain + 1) + j]
                    if chunk_remain < 10:
                ...
                #         self.net_env.players[self.download_video_id - self.offset].user_retent_rate[7 - chunk_remain]))
                # if chunk_remain == 0:
                #     self.conditional_retent_rate[self.download_video_id - self.offset].append(0)
            ...
            if play_video_id == self.last_play_video_id and not switch_flag:
                ...
                if self.download_video_id - self.offset > 0 and not sleep_flag:
                    if chunk_remain >= 10: # 直接读
                        for j in range(10):
                            self.conditional_retent_rate[self.download_video_id - self.offset][j] = float(Players[self.download_video_id - self.offset].user_retent_rate[(-chunk_remain + 1) + j])
                if chunk_remain < 10:
                    self.conditional_retent_rate[self.download_video_id - self.offset].pop(0)
                    self.conditional_retent_rate[self.download_video_id - self.offset].append(0)
        ...
        print('当前决策为下载视频', self.download_video_id, '，sleep_tiem=', sleep_time, '码率等级为',bit_rate,'buffer',self.newstate[0,15:20])
